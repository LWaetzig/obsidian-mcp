/**
 * Note management tools exposed to MCP clients.
 *
 * Covers the full lifecycle of vault notes: listing, reading, creating,
 * updating, appending, deleting, moving, and fetching metadata.
 * Each tool maps directly to a {@link VaultService} method.
 *
 * **Tools registered here:**
 * - `obsidian_list_notes` — paginated note listing, optionally scoped to a folder
 * - `obsidian_read_note` — full content of a single note
 ...
 * - `obsidian_append_to_note` — append content non-destructively
 * - `obsidian_delete_note` — permanently remove a note
 * - `obsidian_move_note` — move or rename a note
 * - `obsidian_get_note_metadata` — frontmatter and tags without loading the body
 *
...
/** Register note management tools on the MCP server. */
